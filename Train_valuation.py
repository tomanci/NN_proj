...
def train(Generator,Discriminator,batch_size,n_epochs,image_file,lr):
    ...
    image_file -> file containing the real images 
    ...
    best_epoch_G = -1
    best_epoch_D = -1
    dataset = image_file #potrebbe dare problemi qui e nei metodi di dataset() 
    ...
    for e in range(0,n_epochs):
        ...
        dataset, _ = dataset.shuffled_dataset()
        ...
        while not last_batch:
            ...
            #fake batch creation
            x_gen_input= gen_net.input_creation(length_dataset = length_dataset , batch_size = batch_size)

            #fake image generated by Generator
            image_output_fake,label_fake_img = gen_net.forward_G(x_gen_input)

            #true image generation
            mini_batch_image_real, mini_batch_label_real, last_batch_flag = dataset.mini_batch_creation(batch_size)
            ...
            mini_batch_img_comb, mini_batch_lab_comb  = dis_net.combined_True_Fake(
                ...
                true_labels = mini_batch_image_real , true_images = mini_batch_label_real
            )
...
def separation(predicted_label):
    ...
    length = output_label.shape[1]

    fake_label = output_label[0][length:]
