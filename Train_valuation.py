...
def train(Generator,Discriminator,batch_size,n_epochs,dataset,lr):
    ...
    optimizer_gen = torch.optim.Adam([p for p in gen_net.parameters() if p.requires_grad], lr, maximize=True) #torch.optim.Adam(filter(lambda p: p.requires_grad, self.net.parameters()), lr)
    optimizer_dis = torch.optim.Adam([p for p in dis_net.parameters() if p.requires_grad], lr, maximize=False)
    ...
    for e in range(0,n_epochs):
        ...
        while not last_batch:
            ...
            x_gen_input,_ = gen_net_2.input_creation(length_dataset = length_dataset , batch_size = batch_size)
